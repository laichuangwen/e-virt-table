...
| ENABLE_TEXT_SELECTION | 启用单元格内文字选中与复制（支持表头、body、footer）；在单元格内拖拽选中文本后，可使用 `Ctrl/Cmd+C` 复制选中内容。有文字选中时优先复制文字，不会触发区域单元格复制；编辑或查找模式下不可用 | boolean | true |
...
| formatterFinderValue | 自定义查找文本，与单元格文本合并搜索 | ^[Function]`({row, column, rowIndex, colIndex,value})=>string\|void` | — |
