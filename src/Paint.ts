...
export class Paint {
  ...
  drawText(
    ...
    const { _text, ellipsis } = this.handleEllipsis(text, width, padding);
    this.ctx.fillText(_text, xPos, yPos);
    this.ctx.restore();
    return ellipsis;
  }
  private handleEllipsis(text: string, width: number, padding = 0) {
    let ellipsis = false;
    let _text = text;
