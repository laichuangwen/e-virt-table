...
export default class ContextMenu {
    ...
    private init() {
        ...
        this.ctx.on('cellContextMenuClick', async (cell: Cell, e: MouseEvent) => {
            ...
            let list: MenuItem[] = CONTEXT_MENU.map((item) => ({
                ...item,
                label: item.label || this.ctx.locale.getText(item.value),
            }));
            list = [...list, ...CUSTOM_BODY_CONTEXT_MENU];
        ...
        this.ctx.on('cellHeaderContextMenuClick', async (cell: CellHeader, e: MouseEvent) => {
            ...
            let list: MenuItem[] = HEADER_CONTEXT_MENU.map((item) => ({
                ...item,
                label: item.label || this.ctx.locale.getText(item.value),
            }));
            list = [...list, ...CUSTOM_HEADER_CONTEXT_MENU];
