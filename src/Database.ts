...
export default class Database {
  ...
  private initData(dataList: any[], level: number = 0) {
    dataList.forEach((item, index) => {
      ...
      const rowKey =
        _rowKey !== undefined && _rowKey !== null
          ? _rowKey
          : generateShortUUID();
  ...
  getItemValueForRowIndexAndColIndex(rowIndex: number, colIndex: number) {
    const has =
      this.rowIndexRowKeyMap.has(rowIndex) && this.colIndexKeyMap.get(colIndex);
    if (!has) {
      return null;
    }
    const rowKey = this.rowIndexRowKeyMap.get(rowIndex);
  ...
  getCheckedState() {
    ...
    const indeterminate =
      totalSelectable && totalSelectable > totalChecked && totalChecked > 0;
